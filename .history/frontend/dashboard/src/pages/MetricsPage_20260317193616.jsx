 ...
 * - Deterministic
 * - Compliance-grade
 * - Active key shown for correlation context ONLY
 */
export default function AuditPage() {
  ...
  useEffect(() => {
    async function loadAudit() {
      try {
        ...
        const res = await apiGet("/api/telemetry/replay");
        setAudit(res);
      } catch (err) {
  ...
  if (!audit || !audit.timelines || Object.keys(audit.timelines).length === 0) {
    return (
      <EmptyState
        title="No Audit Data"
  ...
  return (
    <div className="space-y-8">
        ...
        {activeKey ? (
          <div className="space-y-1">
            ...
            <div className="text-xs text-gray-500">
              Displayed for audit correlation only — no operational control
            </div>
          </div>
        ) : (
          <div className="text-yellow-400">
            ⚠ No active key — audit view is system-wide only
          </div>
        )}
      </div>
      ...
      <div className="space-y-6">
        {Object.entries(audit.timelines).map(
          ([keyId, timeline]) => (
            <div
              key={keyId}
