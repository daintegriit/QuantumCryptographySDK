...
export default function HowItWorksPage() {
  ...
  return (
    <div className="space-y-12 max-w-4xl">
      <div>
        <h1 className={`text-2xl font-bold flex items-center gap-3 ${theme.panelTitle}`}>
          <FaAtom className="text-cyan-400" /> How It Works
        </h1>
      ...
      <Section theme={theme} n="1" title="The Quantum Threat" color="red">
        <p className={theme.mutedText}>Classical computers secure the internet using hard math problems — factoring large numbers (RSA) or solving discrete logarithms (ECC). These problems take billions of years on today's computers.</p>
        <p className={`${theme.mutedText} mt-3`}>Quantum computers running <strong className="text-red-400">Shor's Algorithm</strong> can solve these problems in hours. A sufficiently powerful quantum computer would instantly break every RSA and ECC key protecting HTTPS, banking, email, and government infrastructure.</p>
        <Callout theme={theme} type="danger">
          <strong>Harvest Now, Decrypt Later:</strong> Adversaries are already recording encrypted traffic today to decrypt it once quantum computers arrive. Data encrypted with RSA/ECC is already compromised if it needs to remain secret beyond ~2030–2035.
        </Callout>
      </Section>

      <Section theme={theme} n="2" title="Post-Quantum Cryptography" color="cyan">
        <p className={theme.mutedText}>Post-quantum cryptography (PQC) uses mathematical problems that are hard for <em>both</em> classical and quantum computers. NIST ran a decade-long competition and standardized four algorithms in 2024:</p>
        <div className="mt-4 grid grid-cols-1 md:grid-cols-2 gap-3">
          {[
            ...
            <div key={name} className={`${theme.panel} p-4 rounded-xl border border-${color}-500/20`}>
              <div className={`font-semibold text-${color}-400 text-sm flex items-center gap-2`}><FaShieldAlt className="text-xs" />{name}</div>
      ...
      <Section theme={theme} n="3" title="How Key Encapsulation (KEM) Works" color="cyan">
        <p className={theme.mutedText}>KEM replaces RSA encryption. Instead of encrypting data directly with a public key, it establishes a shared secret that both parties use to derive a symmetric key (AES-256).</p>
        <div className="mt-6 space-y-3">
          {[
            ...
            <div key={step} className="flex gap-4 items-start">
              <div className={`shrink-0 w-7 h-7 rounded-full bg-${color}-500/20 text-${color}-400 text-xs flex items-center justify-center font-bold`}>{step}</div>
              <div className="flex-1 text-sm"><span className={`font-semibold text-${color}-400`}>{actor}: </span><span className={theme.mutedText}>{action}</span></div>
            </div>
          ))}
        </div>
        <Callout theme={theme} type="info">
          QuantumShield uses <strong className="text-cyan-400">Kyber (ML-KEM)</strong> for KEM operations, with AES-256-GCM for the symmetric layer. The Rust crypto core handles encapsulation/decapsulation at ~27,000 operations/second.
        </Callout>
      </Section>

      <Section theme={theme} n="4" title="How Digital Signatures Work" color="purple">
        <p className={theme.mutedText}>Signatures prove authenticity and integrity — that a message came from who it claims, and hasn't been tampered with. Post-quantum signatures replace RSA-PSS and ECDSA.</p>
        <div className="mt-6 space-y-3">
          {[
        ...
        <Callout theme={theme} type="info">
          QuantumShield supports <strong className="text-purple-400">Dilithium (ML-DSA)</strong>, <strong className="text-orange-400">Falcon</strong>, and <strong className="text-green-400">SPHINCS+</strong> for signatures.
        </Callout>
      </Section>

      <Section theme={theme} n="5" title="QuantumShield Architecture" color="green">
        <p className={theme.mutedText}>QuantumShield is a multi-layer cryptographic governance platform:</p>
        <div className="mt-6 space-y-2">
          {[
            ...
            <div key={layer} className={`${theme.panel} p-4 rounded-xl flex gap-4`}>
              <div className={`shrink-0 text-${color}-400 mt-0.5`}>{icon}</div>
              <div>
      ...
      <Section theme={theme} n="6" title="Key Lifecycle" color="yellow">
        <p className={theme.mutedText}>Every cryptographic key has a lifecycle. QuantumShield tracks and enforces it automatically.</p>
      ...
      <Section theme={theme} n="7" title="Policy Drift Detection" color="purple">
        <p className={theme.mutedText}>Policy drift is the silent degradation of cryptographic governance over time — rising denial rates, new unapproved schemes appearing, or keys going unrotated.</p>
        <p className={`${theme.mutedText} mt-3`}>QuantumShield compares a baseline window (e.g. last 90 days) against a recent window (e.g. last 14 days). If deny rates rise, new schemes appear, or rotation frequency drops, drift is flagged before it becomes a compliance failure.</p>
        <Callout theme={theme} type="warn">Policy drift is often invisible until an audit. QuantumShield surfaces it continuously so governance teams can act before regulators do.</Callout>
      </Section>

      <Section theme={theme} n="8" title="Quantum Safety Simulation" color="cyan">
        <p className={theme.mutedText}>The simulation engine projects each key's cryptographic durability year-by-year based on estimated quantum break timelines from the NIST PQC registry.</p>
        ...
        <p className={`${theme.mutedText} text-xs mt-4`}>Simulation scenarios: Conservative (2035), Baseline (2040), Aggressive (2030), Breakthrough (2028).</p>
      </Section>
...
function Callout({ theme, type, children }) {
  const styles = { info: "border-cyan-500/30 bg-cyan-500/5 text-cyan-300", warn: "border-yellow-500/30 bg-yellow-500/5 text-yellow-300", danger: "border-red-500/30 bg-red-500/5 text-red-300" };
  const icons  = { info: <FaShieldAlt />, warn: <FaExclamationTriangle />, danger: <FaBolt /> };
  return (
    <div className={`mt-4 p-4 rounded-xl border text-sm flex gap-3 ${styles[type]}`}>
